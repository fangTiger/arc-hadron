...
窄屏下筛选 chips 可在自身区域横向滚动，资产表 SHALL 只在自身滚动容器内溢出，页面 MUST NOT 被筛选或表格撑宽。LIVE 事件条在宽屏可固定于视口底部并预留内容安全区，在窄屏 MUST 进入普通文档流或使用等效的不遮挡方案；任何视口下都不得覆盖资产表交互与内容。
...
#### Scenario: 手机浏览市场表
- **WHEN** 用户以 390px 视口访问首页并查看多列资产数据
- **THEN** 页面本身无横向滚动，资产表可在局部滚动框内查看其余列，LIVE 事件条不覆盖表格

### Requirement: 资产详情页
详情页 SHALL 展示：资产档案与元数据文档、**Issuer 跳转入口（header 或 sidebar 显示 `Issuer: <displayName>` 可点击跳转 `/issuers/[slug]`）**、一级购买面板（价格/余量）、二级挂单表、价格走势图（lightweight-charts）、交易历史表（每条含 Arc explorer 链接）。详情页 SHALL 保持既有交易优先顺序；加载态 MUST 使用与桌面三栏、移动单列相匹配的响应式占位高度，不得在窄屏产生页面级横向滚动或连续超长空白块。
...
#### Scenario: 手机加载资产详情
- **WHEN** 用户以 390px 视口打开资产详情且链上数据仍在加载
- **THEN** 页面显示紧凑单列 skeleton，所有占位块保持在视口内并近似最终内容顺序
