...
interface ListForSaleIntent {
  tokenId: bigint;
  amount: bigint;
  pricePerShare: bigint;
}

interface ListForSaleLocalState {
...
export interface UseListForSaleResult {
  listForSale: (tokenId: bigint, amount: bigint, pricePerShare: bigint) => void;
  status: ListForSaleStatus;
  txHash?: `0x${string}`;
  approveTxHash?: `0x${string}`;
  errorText?: string;
...
function deriveListForSaleState({
  approvalReceipt,
  listReceipt,
  localState,
}: {
  approvalReceipt: ReceiptState;
  listReceipt: ReceiptState;
  localState: ListForSaleLocalState;
  ...
  if (localState.status === "pending") {
    const receiptState = deriveReceiptStatus(listReceipt);
...
export function useListForSale(): UseListForSaleResult {
  const { address } = useAccount();
  const [localState, setLocalState] = useState<ListForSaleLocalState>({ status: "idle" });
  const busyRef = useRef(false);
  const intentRef = useRef<ListForSaleIntent | null>(null);
  ...
  const listReceiptQuery = useWaitForTransactionReceipt({
    hash: txHash,
    query: {
  ...
  const derivedState = useMemo(
    () =>
      deriveListForSaleState({
        ...
        listReceipt: {
          error: listReceiptQuery.error,
          isError: listReceiptQuery.isError,
          status: listReceiptQuery.data?.status,
        },
        localState,
      }),
    [
      ...
      listReceiptQuery.data?.status,
      listReceiptQuery.error,
      listReceiptQuery.isError,
      localState,
    ],
  );
    ...
    writeContract(
      {
        ...
        functionName: "list",
        args: [intent.tokenId, intent.amount, intent.pricePerShare],
      },
      {
  ...
  const listForSale = useCallback(
    (tokenId: bigint, amount: bigint, pricePerShare: bigint) => {
      if (
      ...
      // 先以链上授权状态为准，再决定是否发起两步交易流。
      intentRef.current = { tokenId, amount, pricePerShare };
    ...
    [address, failWithError, refetchApproval, status, submitApproval, submitList],
  );
