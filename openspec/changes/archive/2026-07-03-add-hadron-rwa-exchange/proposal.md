...
- **Vercel 部署（原 5.2）排除**：2026-07-03 用户明确指示"排除 vercel 部署"，README 保留并已交付。
- **3D 首屏（原 5.1 可选项）跳过**：性能与交易所审美风险大于收益，用户未要求。
- **范围新增（验收反馈驱动，M3R/M4）**：小数份额粒度（SHARE_SCALE=100，V3 重发行 tokenId 15-28）、事件层 localStorage 持久化缓存、行级导航、成交对手方追踪列、详情页 BUY|SELL 双 tab——均已实现并计入 specs/ 当前真相。
- 后续路线（另立提案）：AI 层（DeepSeek）→ 买单深度 → 收益分配。
