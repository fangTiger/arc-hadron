...
event BidCancelled(uint256 indexed bidId, uint256 indexed tokenId, address indexed bidder, uint256 returnedAmount);
...
**用例清单（先 RED）：** 成功挂单（事件/托管余额）；付款不符 revert；零价/零量 revert；EOA 直接通过 receiver 门；成功部分成交（份额/货款/费/余量断言）；两次部分成交至归零后 active=false；未授权 fill revert；零量/超量 fill revert；已撤销 bid fill revert；非 bidder 撤单 revert；部分成交后撤单退款额准确；全量成交后撤单 revert；费向下取整边界（totalPaid=1 wei 级）；getBid 不存在 id revert；bidsByToken 只含活跃有余量项。
...
- 用例：无 receiver 合约 placeBid 被 `BidderNotReceiver` 拒；supportsInterface=true 但回调 revert 的 bid 存在时 fillBid revert 且市场状态一致（假深度残余风险实证）；回调重入 fillBid/cancelBid 被 nonReentrant 拒且余量/托管一致；拒收退款 bidder 的 cancelBid revert 只伤自己（他人挂单/买单不受影响）
...
## 组 2：部署与播种（codex 写脚本，Claude 执行广播）

### Task 2.1 `contracts/script/SeedV4.s.sol`（codex）

- 合并 SeedV3 + SeedSecondary + SeedTrades 职责为单脚本（先读三者理解播种账户与数据形态），并新增：每资产 2-4 档买单（价格低于最优卖价、含不同 bidder）+ 2-3 笔 BidFilled 成交样本（进价格序列）
- 旧 Seed 脚本保留不动
- **验证：** `forge script script/SeedV4.s.sol --fork-url $ARC_RPC`（dry-run 无 revert）

### Task 2.2 重部署 + env 同步（Claude 亲自，需广播密钥）

- `forge script script/Deploy.s.sol --rpc-url ... --broadcast` → 新地址写 `contracts/deployments/arc-testnet.json`
- SeedV4 广播
- web 同步：`web/.env.local` 与 `.env.example` 的 `NEXT_PUBLIC_*` 地址 + `NEXT_PUBLIC_DEPLOY_BLOCK`；`web/lib/chain.ts` FIRST_ACTIVE_TOKEN_ID 复核；`web/test` 中地址常量复核；ABI 重新生成/拷贝（沿现有惯例，先查 `web/lib/contracts.ts` ABI 来源）
- **验证：** dev server 三页 200 且市场数据可见（新地址读取正常）→ 勾选 2.1-2.3 → 提交

---

## 组 3：前端事件层与数据（codex，web TDD）

### Task 3.1 事件解码（`web/lib/events.ts` + `web/test/` 对应测试）
...
### Task 3.2 成交口径三处同步 + label

- `events.ts` 成交过滤、`web/lib/marketMetrics.ts`、`web/lib/ai/snapshot.ts` TRADE_TYPES 同步加 `bid-filled`；一致性测试（三处对同一事件集口径一致）
...
### Task 3.3 `web/lib/bids.ts` + 读 hooks（`web/lib/hooks/useBids.ts`）
...
**组 3 验证：** `cd web && npm run test`（零回归）+ lint → 勾选 3.1-3.3 → 提交。

---

## 组 4：交易 hooks 与 UI（codex，web TDD）

### Task 4.1 交易 hooks（`web/lib/hooks/usePlaceBid.ts` / `useCancelBid.ts` / `useFillBid.ts`）
...
### Task 4.2 资产页（`web/components/asset/BidsTable.tsx` + `PlaceBidPanel.tsx` + 挂载 `web/app/asset/[id]/page.tsx`）
...
### Task 4.3 持仓页（My Bids 区块 + "Sell to bid" 入口 + 挂载）

- My Bids：表格（资产/价格/余量/托管额/Cancel 按钮），空态英文文案
- 持有资产行增 "Sell to bid" 入口 → 打开该资产 Fill 流程（复用 4.1/4.2 组件）

### Task 4.4 活动流文案（若 3.2 未覆盖处补齐）

**组 4 验证：** 全量 test + lint（UI 文案英文、注释中文）→ 勾选 4.1-4.4 → 提交。
...
- spec 覆盖：delta 4 ADDED + 1 MODIFIED 需求分别落 Task 1.1（托管/成交/撤单）、1.2+MODIFIED（对抗）、3.x+4.x（可读接口与前端呈现、三处口径）✓
- 类型一致性：BidView/事件字段/hook 签名在 3.x 与 4.x 间一致 ✓
- 占位符：无 TBD；组 2 ABI 来源与播种账户标注了"实现时先读现有文件"，属探索性前置而非缺口 ✓
