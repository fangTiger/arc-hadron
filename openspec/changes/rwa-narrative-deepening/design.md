...
- 新增 `sovereign-bonds` + `corporate-bonds` 两类，+ 4 资产 + 3 新 issuer。
...
| 2 | 新增类别 | sovereign-bonds + corporate-bonds |
...
| 10 | 归并结果 | 现有 14 资产 → 9 issuer；新增 4 资产 → 3 新 issuer；共 12 issuer |
...
- **arc-testnet 广播**：追加 4 笔 createAsset，按 memory [[arc-testnet-broadcast-practice]] 用 `--slow` + 一次性密钥。
...
5. 新增 4 资产 JSON + 3 issuer JSON + Foundry 种子脚本
6. 用户手动执行 arc-testnet 广播（`--slow` + 一次性密钥）

回滚：前端纯代码 git revert；多出 4 个 asset id 无害。

## Open Questions

- Sovereign issuer 拆一家还是两家 —— 默认拆 `germany-treasury-demo` + `japan-treasury-demo`，Claude 审 diff 时可微调。
